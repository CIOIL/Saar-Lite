Ext.define('ExtDoc.tools.controllers.ExtDocClipboardCopyController', {
	extend: 'ExtDoc.tools.controllers.ExtDocAbstractActionController',
	alias: 'controller.clipboardCopyController',
	copyAction: function(){
		...
		completeUrl = ExtDoc.config.ExtDocConfig.restUrl + 'os/copyobjects';
		...
		xhr.onreadystatechange = function(e){
			if (this.readyState == 4 && this.status == 200)
			{
				...
				var clipboardCopyTitle = ExtDoc.locales.ExtDocLocaleManager.getText('copy');
				var clipboardCopyWarning = ExtDoc.locales.ExtDocLocaleManager.getText(selectedRecords > 1 ? 'copy_success_m' : 'copy_success_s');
				...
				if (errors && errors.length > 0)
				{
					clipboardCopyWarning += ' ' + ExtDoc.locales.ExtDocLocaleManager.getText('partial') + '<br />' + ExtDoc.locales.ExtDocLocaleManager.getText('clipboard_warning');
					...
					for (var i = 0; i < errors.length; i++)
					{
						...
						clipboardCopyWarning += '<br />' + number + '. ' + eDesc + ': ' + eDocName;
					}
				}
				
				Ext.toast({
					autoCloseDelay: toastTimeout,
					html: clipboardCopyWarning,
					title: clipboardCopyTitle,
					rtl: toastRtl
				});
			}
	...
	buildJson: function (ids, copyToId){
		var preJson = {
			"properties": {
				"objectIds": ids,
				"copyToId": copyToId
