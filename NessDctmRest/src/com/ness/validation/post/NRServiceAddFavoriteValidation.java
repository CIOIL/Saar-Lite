...
public class NRServiceAddFavoriteValidation extends NRValidation implements INRValidation
{
	...
	public NRJsonObject validate(Object objectToValidate) throws Exception
	{
		...
		validationObject = NRValidationUtills.validateFieldsArray(new NRValidationUtills.ValidateDocumentFolderIdsCommand(), object, folderIdsFieldsArr, NRConstants.VE_NOT_VALID_OBJECT_IDS);
		...
		public boolean execute(List<Object> updateObjects)
		{
			...
			if(updateObjects.size() > 0)
			{
				for (Object element : updateObjects)
				{
					...
					if (!NRValidationUtills.isValidFolderId(object.get(NRConstants.R_OBJECT_ID).toString()))
					{
						result = false;
						break;
					}
